...
public class PlayerController : MonoBehaviour
{
    ...
    void HandleInputs()
    {
        ...
        if (Input.GetButtonDown("A") && _attackForm) StartCoroutine(Attack());
    ...
    {
        if (collision.collider.CompareTag("Enemy A") && !_isInvulnerable)
        {
            EnemyAController enemyA = collision.collider.GetComponent<EnemyAController>();
            if (enemyA != null)
            {
                EnemyAData _enemyAData = GameManager.Instance.GetEnemyAData(enemyA.id);

                HealthSystem healthSystem = GetComponent<HealthSystem>();
                if (healthSystem != null)
                    healthSystem.TakeDamage(_enemyAData.meleeDamage);

                StartCoroutine(ApplyKnockback());
            }
        }
        if (collision.collider.CompareTag("Enemy B") && !_isInvulnerable)
        {
            EnemyBController enemyB = collision.collider.GetComponent<EnemyBController>();
            if (enemyB != null)
            {
                EnemyBData _enemyBData = GameManager.Instance.GetEnemyBData(enemyB.id);

                HealthSystem healthSystem = GetComponent<HealthSystem>();
                if (healthSystem != null)
                    healthSystem.TakeDamage(_enemyBData.meleeDamage);
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        {
            ItemData.ITEM itemType = ItemController.itemType;

            switch (itemType)
            {
                case ItemData.ITEM.ItemA:
                    _attackForm = true;
                    _projectileForm = false;
                    _bombForm = false;
                    break;
                case ItemData.ITEM.ItemB:
                    _attackForm = false;
                    _projectileForm = true;
                    _bombForm = false;
                    break;
                case ItemData.ITEM.ItemC:
                    _attackForm = false;
                    _projectileForm = false;
                    _bombForm = true;
                    break;
        ...
        if (collider.CompareTag("Enemy Bomb") && !_isInvulnerable)
        {
            EnemyBombController enemyBomb = collider.GetComponent<EnemyBombController>();
            if (enemyBomb != null)
            {
                EnemyBombData _enemyBombData = GameManager.Instance.GetEnemyBombData(enemyBomb.id);

                HealthSystem healthSystem = GetComponent<HealthSystem>();
                if (healthSystem != null)
                    healthSystem.TakeDamage(_enemyBombData.bombDamage);
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label($"_attackForm = {_attackForm}");
